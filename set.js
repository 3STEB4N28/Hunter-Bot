...
{field:Command Name:\`set modlogs $message[2]\`:no}
...
$if[$message[1] $message[2]==modlogs on]
	$setServerVar[logsst;on;$guildID]
	$setServerVar[logswebhookid;$splitText[1]]
	$setServerVar[logswebhooktoken;$splitText[2]]
	$textSplit[$createWebhook[$findChannel[$message[3]];Hunter Bot Logs;$userAvatar[$clientID];yes;,];,]


	$reply[$messageID;{title:ModLogs}
	{description:I will now send every moderation logs to <#$findChannel[$message[3]]>!}
	{footer:$getVar[botfooter]}
	{color:BLACK};no]
	
	$onlyBotPerms[managewebhooks;{title:Error}
	{description:I need the \`MANAGE_WEBHOOKS\` permission to execute this command!}
	{footer:$getVar[botfooter]}
	{color:BLACK}]
	$onlyIf[$findChannel[$message[3];no]!=undefined;{title:Error}
	{description:I could'nt find that channel!}
	{footer:$getVar[botfooter]}
	 {color:BLACK}]
	...
	{footer:$getVar[botfooter]}
	{color:BLACK}]
	$onlyIf[$getServerVar[logsst]==off;{execute:eerror}]

	$elseif[$message[1] $message[2]==modlogs off]
	$deleteWebhook[$getServerVar[logswebhookid];$getServerVar[logswebhooktoken]]
	$setServerVar[logsst;off]
	$setServerVar[logswebhooktoken;$getVar[logswenhookid]]
	$setServerVar[logswebhookid;$getVar[logswebhookid]]
	
$reply[$messageID;{title:ModLogs}
{description:Moderation logs have been successfully disabled!}
{footer:$getVar[botfooter]}
{color:BLACK};no]

	$onlyIf[$getServerVar[logsst]==on;{execute:derror}]
	$endelseif
	$endif
	$onlyIf[$message[1]==modlogs;]
...
{footer:$getVar[botfooter]}
{color:BLACK}]
$onlyPerms[managechannels;{title:Error}
{description:You're missing the following permission \`MANAGE_CHANNELS\`!}
{footer:$getVar[botfooter]}
{color:BLACK}]`
}, {
	name: "set", 
